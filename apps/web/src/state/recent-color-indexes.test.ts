...
describe("rememberColorIndexIndex (design system, Mobile : couleurs récentes)", () => {
  // Quatre couleurs à portée de pouce dès l'arrivée
  it("starts with four colors", () => {
    expect(INITIAL_RECENT_COLOR_INDEXES).toHaveLength(4);
  });
  ...
  it("puts a new color first and drops the oldest", () => {
    expect(rememberColorIndex([5, 28, 19, 9], 12)).toEqual([12, 5, 28, 19]);
  });

  // Une couleur déjà là ne bouge pas : leur ordre est stable
  it("keeps the order when the color is already there", () => {
    expect(rememberColorIndex([5, 28, 19, 9], 19)).toEqual([5, 28, 19, 9]);
  });

  // La gomme n'est pas une couleur : elle n'entre jamais
  it("never remembers the eraser", () => {
    expect(rememberColorIndex([5, 28, 19, 9], TRANSPARENT_COLOR_INDEX)).toEqual([5, 28, 19, 9]);
  });
});
